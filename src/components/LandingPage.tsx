...
interface LandingPageProps {
  ...
  onOpenBrainstorm?: (code?: string) => void;
}
...
export const LandingPage: React.FC<LandingPageProps> = ({ onStartBuilding, onOpenBrainstorm }) => {
  ...
  const handleJoinChat = () => {
    if (joinRoomCode.trim().length === 6) {
      // In a real app, we'd navigate or trigger the modal/chat
      // For this implementation, we'll use the onOpenBrainstorm to trigger the modal
      // but we might need a more direct way if we want to bypass the modal
      if (onOpenBrainstorm) {
        onOpenBrainstorm(joinRoomCode.trim());
      }
    }
  };
          ...
          <div className="flex justify-between h-20 items-center">
            <a className="flex items-center gap-3 group focus:outline-none rounded-lg px-2 py-1" href="#">
              ...
              <div className="flex flex-col">
                <span className="font-display font-bold text-xl tracking-tight text-white group-hover:text-primary transition-colors">API Roulette</span>
          ...
          <div className="flex flex-col lg:flex-row items-center gap-16">
            <div className="lg:w-1/2 text-left animate-fade-in-up">
              ...
              <p className="text-lg text-slate-400 mb-10 leading-relaxed max-w-xl border-l-2 border-primary/30 pl-6">
                Stop choosing between speed and structure. API Roulette generates production-ready, full-stack scaffolding in seconds, giving you the agility to win hackathons and the architecture to scale businesses.
              </p>
              ...
              {/* Join Room UI */}
              <div className="mt-8 p-4 bg-[#05110a] border border-white/10 rounded-2xl max-w-sm">
                <div className="flex gap-2">
                  <input
                    ...
                    maxLength={6}
                    className="flex-1 bg-[#0b1f15] border border-white/10 rounded-lg px-4 py-2 text-white font-mono text-center tracking-widest focus:ring-1 focus:ring-primary focus:border-transparent outline-none"
                  />
                  <button
                    onClick={handleJoinChat}
                    className="bg-accent hover:bg-accent/80 text-white px-4 py-2 rounded-lg font-bold transition-all"
                  >
                    Join Room
                  </button>
                </div>
                ...
                <div className="relative bg-[#05110a] rounded-2xl border border-white/10 shadow-2xl overflow-hidden backdrop-blur-sm">
                  <div className="bg-[#020604] border-b border-white/5 px-4 py-3 flex items-center justify-between">
                    ...
                    <div className="text-[10px] font-mono text-slate-500 flex items-center gap-2">
                      <i className="fa-solid fa-shield-halved text-primary"></i> api-roulette-engine --turbo
                    </div>
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      ...
                      className={`px-3 py-1.5 rounded border text-xs transition ${addons.tailwind
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded border text-xs transition ${addons.docker
                        ? 'border-primary/30 bg-primary/10 text-primary hover:bg-primary/20'
                        : 'border-white/10 bg-[#020604] text-slate-400 hover:border-white/30'
                        }`}
                    >
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded border text-xs transition ${addons.cicd
                        ? 'border-primary/30 bg-primary/10 text-primary hover:bg-primary/20'
                        : 'border-white/10 bg-[#020604] text-slate-400 hover:border-white/30'
                        }`}
                    >
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded border text-xs transition ${addons.prisma
                        ? 'border-primary/30 bg-primary/10 text-primary hover:bg-primary/20'
                        : 'border-white/10 bg-[#020604] text-slate-400 hover:border-white/30'
                        }`}
                    >
            ...
            <div className="md:col-span-8 bg-[#010804] p-6 font-mono text-sm relative overflow-hidden">
              ...
              <div className="space-y-2 font-mono text-xs sm:text-sm">
                <div className="flex gap-2">
                  ...
                  <span className="text-slate-400">$ api-roulette create saas-starter --turbo</span>
                </div>
                <div className="py-2 text-slate-500 italic">Initializing API Roulette Engine v2.4...</div>
      ...
      <section className="py-24 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-4 relative z-10 text-center">
          ...
          <div className="mt-8 font-mono text-xs text-slate-500">
            <span className="bg-[#05110a] px-3 py-1 rounded border border-white/10 text-primary">npm install -g api-roulette</span>
          </div>
        </div>
      </section>
      ...
      <footer className="bg-[#010804] border-t border-white/5 pt-16 pb-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
            <div className="col-span-1 md:col-span-1">
              <div className="flex items-center gap-3 mb-6">
                ...
                <span className="font-bold text-xl text-white">API Roulette</span>
              </div>
          ...
          <div className="border-t border-white/5 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-xs text-slate-600">© 2023 API Roulette. All rights reserved.</p>
