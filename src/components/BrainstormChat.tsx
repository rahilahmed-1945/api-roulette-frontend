...
import { BrainIcon, XIcon, ClipboardIcon, SendIcon, ServerIcon, RefreshIcon } from './Icons';
...
export const BrainstormChat: React.FC<BrainstormChatProps> = ({ roomCode, onClose }) => {
  ...
  const currentUsername = localStorage.getItem('brainstorm_username') || 'Anonymous';
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <p className="text-slate-400 text-sm">Collaborative Planning</p>
              <span className="w-1 h-1 bg-slate-600 rounded-full"></span>
              <p className="text-primary text-sm font-medium">{participantCount} {participantCount === 1 ? 'Participant' : 'Participants'}</p>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => {
              ...
              const fullUrl = `${origin}/brainstorm/${roomCode}`;

              // Log to console for dev visibility
              console.log('Sharing URL:', fullUrl);

              // Always show alert first
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
        {messages.map((message) => {
          if (message.type === 'system') {
            return (
              <div key={message.id} className="flex justify-center my-4">
                <div className="bg-white/5 border border-white/5 px-4 py-1.5 rounded-full text-xs text-slate-400 font-medium">
                  {message.content}
                </div>
              </div>
            );
          ...
          return (
            ...
            >
              <div
                className={`max-w-[80%] rounded-lg p-4 ${message.sender === currentUsername
              ...
              >
                ...
                <div
                  className={`prose prose-sm max-w-none ${message.sender === currentUsername
                    ? 'text-[#010804]'
                    : 'text-white'
                    }`}
                  dangerouslySetInnerHTML={{ __html: formatMessage(message.content) }}
                />
                <div className={`text-xs opacity-60 mt-2 ${message.sender === currentUsername
                  ? 'text-[#010804]'
                  : 'text-white'
                  }`}>
      ...
      <div className="bg-[#05110a] border-t border-white/10 p-4">
        <div className="flex gap-3">
          <textarea
            ...
            placeholder="Discuss project planning, API integration, or team collaboration..."
        ...
        <div className="text-xs text-slate-500 mt-2 text-center">
          Press Enter to send • Shift+Enter for new line • Real-time collaboration enabled
        </div>
      </div>
    </div >
  );
